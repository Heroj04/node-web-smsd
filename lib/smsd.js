...
function parseGSM(gsm) {
  const headers = {};
  let payload = null;
  const payloadSplit = gsm.indexOf(Buffer.from('\n\n', 'latin1'));
...
async function readGSMDir(directory) {
  ...
  // Set up our internal function
  const parseFunc = async (filePath) => {
  ...
  files.forEach((filePath) => {
    const promise = parseFunc(path.join(directory, filePath));
